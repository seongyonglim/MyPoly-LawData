...
# -*- coding: utf-8 -*-
"""
로컬 데이터를 Cloud SQL로 직접 마이그레이션 (Python으로)
인코딩 문제 없이 안전하게 데이터 전송
"""
...
# Cloud SQL 설정 (VM의 Cloud SQL Proxy를 통해 연결)
# VM에서 실행할 때는 127.0.0.1 사용 (Cloud SQL Proxy가 로컬에서 리스닝)
CLOUD_DB = {
    'host': '127.0.0.1',  # Cloud SQL Proxy (VM에서 실행 시)
    'database': 'mypoly_lawdata',
    'user': 'postgres',
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("로컬 → Cloud SQL 데이터 마이그레이션 (Python 직접 연결)")
    print("=" * 80)
    ...
    try:
        ...
        print("\n💡 Cloud SQL 방화벽 규칙에서 로컬 IP를 허용해야 할 수 있습니다.")
        local_conn.close()
        return
